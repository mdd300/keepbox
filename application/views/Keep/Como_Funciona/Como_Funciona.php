
<div class="content-img-top-cf">
    <img class="img-top-cf" src="<?= base_url() ?>public/assets/metronic/custom/img/photo/banner_como_funciona.jpg">
</div>

<div class="content-text-banner-cf">
    <div class="color-text-green text-title text-font-sans" style="position: relative">
        COMO  <div class="color-text-white" >FUNCIONA?</div>
    </div>
</div>
   
        
    </ul>

<div class="text-font-sans content-li-center-vert" >

    <ul class="content-ul-data-serv">
        <li>
           <div class="color-text-green text-title-sm step-text">Como comprar com a KeepBox</div> 
            
        </li>
    </ul>
     <ul class="content-ul-data-serv">
        <li>
            
           <div class="color-text-grey-light" style="padding-top: 10px">1 -  Crie sua conta KeepBox gratuitamente e compre nas lojas online dos EUA. </div> 
        </li>
    </ul>

    <ul class="content-ul-data-serv">
        <li>
            
           <div class="color-text-grey-light" style="padding-top: 10px"
>2 – Mande entregar suas compras no endereço da sua conta KeepBox 
           </div> 
                                        
        
        </li>
    </ul>

    <ul class="content-ul-data-serv">
        <li>
            
           <div class="color-text-grey-light"style="padding-top: 10px" >3 – Recebemos e juntamos suas encomendas em uma única caixa, sem custo adicional
*(Assim, você paga um único frete e pode economizar até 80% no valor de envio)
</div> 
        </li>
    </ul>

    <ul class="content-ul-data-serv">
        <li>
            
           <div class="color-text-grey-light"style="padding-top: 10px" >4 – Enviamos para você, onde quiser, pelos melhores preços 
</div> 
        </li>
    </ul>


</div>

 <div class="content-video-module2-cf color-background-white" style="background-color: red">
            <!---->
            <!--Video aqui-->
            <!---->
 </div> 


 <div style="padding-top: 33%" class="text-font-sans text-title-sm content-align-tutorial-module3">
     <div class="color-text-grey-light">Ainda com dúvidas?&nbsp;  
        <button class="btn-config-2 color-background-roxo color-text-white text-1">Confira Aqui</button></div>
 </div>

 <div class="content-simulator-module4-serv ">
  
        <div class="color-text-roxo text-title-md text-font-sans">COMPRA ASSISTIDA / PERSONAL SHOPPER</div>
        <div class="color-text-roxo text-2 text-font-sans" > Me ajude nas compras</div>

</div>

<div class="text-font-sans content-li-center-vert-less-pad" >

   
     <ul class="content-ul-data-serv">
        <li>
            
           <div class="color-text-grey-light" style="padding-top: 10px">1 – Você nos envia quais itens deseja comprar, quantidade e tamanho.
(a maneira mais fácil é copier e colar os links exatos de cada produto)  
</div> 
        </li>
    </ul>

    <ul class="content-ul-data-serv">
        <li>
            
           <div class="color-text-grey-light" style="padding-top: 10px"
>2 – Nós faremos um orçamento completo para você e lhe enviaremos para aprovação
           </div> 
                                        
        
        </li>
    </ul>

    <ul class="content-ul-data-serv">
        <li>
            
           <div class="color-text-grey-light"style="padding-top: 10px" >3 – Após sua aprovação, iremos às compras para você! Compraremos tudo que você pediu e cadastramos tudo em sua conta KeepBox para você conferir e acompanhar suas compras.
</div> 
        </li>
    </ul>

    <ul class="content-ul-data-serv">
        <li>
            
           <div class="color-text-grey-light"style="padding-top: 10px" >4 – Então, unificaremos tudo que você desejar em uma só caixa, você seleciona os serviços extras que deseja e nós enviamos tudo para seu endereço no Brasil. 
</div> 
        </li>
    </ul>


</div>







</body>
</html>